...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  // popup.js
  var firebaseConfig = {
    apiKey: "AIzaSyDummyKeyForFreeTier",
    // Placeholder for Spark plan
  ...
  var state = {
    ...
    foundWords: [],
    wordFinders: {},
    // Track who found each word { word: nickname }
    score: 0,
    puzzle: null,
    puzzleId: null,
    attributionMode: 0,
    // 0 = none, 1 = colors, 2 = sections
    multiplayer: {
      roomCode: null,
  ...
  var els = {
    ...
    // Restoring dotsContainer which was missing
    cells: {
  ...
  function selectRandomPuzzle() {
    ...
    loadPuzzle(index);
    showMessage("New Random Puzzle!", 1e3);
    if (state.multiplayer.roomCode) {
  ...
  function shuffleLetters() {
    ...
    for (let i = outer.length - 1; i > 0; i--) {
      ...
      [outer[i], outer[j2]] = [outer[j2], outer[i]];
    }
    els.cells.outer.forEach((cell, i) => {
  ...
  async function loadNYTDailyPuzzle(shouldBroadcast = true) {
    showMessage("Fetching NYT Daily...", 2e3);
    try {
      ...
      let wordList = Array.from(doc2.querySelectorAll('a[id^="link-definition-"]')).map((a) => a.id.replace("link-definition-", "").toLowerCase()).filter((w) => w.length >= 4);
      if (wordList.length === 0) {
        wordList = Array.from(doc2.querySelectorAll("#main-answer-list .flex-list-item, #all-answers li, .answer-list li")).map((li) => li.textContent.trim().toLowerCase()).filter((w) => w.length >= 4);
      }
      let letters = [];
      const scriptText = Array.from(doc2.querySelectorAll("script")).map((s) => s.textContent).join(" ");
      ...
      if (letters.length < 7) {
        const titleMatch = doc2.title.match(/([A-Z])\s*\(\s*([A-Z\s]+)\s*\)/i);
        if (titleMatch) {
      ...
      if (letters.length < 7) {
        const lettersText = doc2.querySelector("#today-letters")?.textContent.trim().toUpperCase() || doc2.querySelector(".pangram")?.textContent.trim().toUpperCase();
      ...
      renderFoundWords();
      renderMultiplayerBanner();
      showMessage("NYT Daily Loaded!", 2e3);
      ...
      console.error(err);
      showMessage("Error loading NYT Daily: " + err.message, 3e3);
    }
  }
    ...
    unsubscribeRoom = onSnapshot(roomRef, (snapshot) => {
      ...
      if (data.players) {
        ...
        const playersList = Object.entries(data.players).map(([uid, p]) => {
          ...
          const isOnline = p.online && now - lastActive < 9e4;
          return {
            playerId: uid,
            nickname: p.nickname,
            online: isOnline
        ...
        serverFound.forEach((word) => {
          ...
          if (!state.foundWords.includes(word)) {
            ...
            if (finder !== state.multiplayer.nickname) {
              showMessage(`${finder} found ${word}!`, 2e3);
            }
  ...
  async function syncPuzzleToFirebase(puzzleId) {
    if (!state.multiplayer.roomCode) return;
    const roomRef = doc(db, "rooms", state.multiplayer.roomCode);
    await updateDoc(roomRef, { puzzleId, foundWords: {} });
  }
  ...
  function setupEventListeners() {
    ...
    const editHandler = (e) => {
      ...
      if (newName && newName.trim() !== "") {
        ...
        if (state.multiplayer.roomCode) {
          const roomRef = doc(db, "rooms", state.multiplayer.roomCode);
          const playerKey = `players.${state.playerId}.nickname`;
          updateDoc(roomRef, { [playerKey]: val }).catch((e2) => console.warn("Nickname update failed:", e2));
        }
    ...
    if (els.toggleAttributionBtn) {
      const modeIcons = ["\u{1F464}", "\u{1F3A8}", "\u{1F4CB}"];
    ...
    els.multi.activeRoomCode.addEventListener("click", () => {
      ...
      if (code) {
        navigator.clipboard.writeText(code).then(() => {
          showMessage("Room code copied!", 2e3);
        });
      }
    });
